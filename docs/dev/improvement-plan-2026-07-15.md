...
> **実施状況(2026-07-15 更新)**:
> - バッチ A(第 1・4・8 位)実施済み ✅ — クリップボード書き込みエラー処理、
>   mutex per-user 化、LRESULT/GetMessageW/パイプタイムアウトキャンセルの修正。
> - バッチ B(第 2・7・9 位)実施済み ✅ — genpass の履歴/クラウド同期除外
>   (`sensitive=True` + 3 形式)、hold.txt の DPAPI 暗号化(平文フォールバックは
>   1 リリース限定)、ClipboardGuard 復元ストーム自動解除(15 回/3 秒)。
> - バッチ C(第 3・6 位)実施済み ✅ — pip-audit/OSV の fail 化(実施前に
>   lockfile 更新で検出ゼロ化: pip/urllib3/Pillow/idna/msgpack)、全 Action の
>   SHA ピン統一、カバレッジ分母是正(Windows 実測 81.65%)。
> - テスト 702 件 green。残: 第 5 位(exe メタデータ+署名)、第 10 位(pystray 調査)。
...
## 第 2 位: genpass を Win+V 履歴・クラウド同期から除外(Review #2)✅ 実施済み(2026-07-15)
...
## 第 3 位: セキュリティ CI の実効化(Review #3, #7)✅ 実施済み(2026-07-15)
...
## 第 6 位: カバレッジ分母の是正(Review #8)✅ 実施済み(2026-07-15)
...
- **実測結果(2026-07-15)**: Windows 81.65% / Ubuntu 74.80%(windows_only 79 件が
  skip されるため Ubuntu が律速)。ゲートは暫定 74% に設定。CLI ハンドラ
  (`_cli_config` 28% / `_cli_daemon` 28% / `_cli_dict` 34%)のユニットテスト追加で
  +2% ずつ引き上げるのがフォローアップタスク。

**工数**: 1 日 / **リスク**: CI が一時的に赤くなる可能性 → ブランチで実測してから調整。

---

## 第 7 位: hold.txt の DPAPI 暗号化(Review #6)✅ 実施済み(2026-07-15)
...
## 第 9 位: ClipboardGuard 復元レート制限(Review #10)✅ 実施済み(2026-07-15)
